    ...
    <div x-data="tocItem" x-init="state(`{{ $toc_item->markdown_item->slug }}`)" class="relative">
        @if (!$toc_item->markdown_item->is_draft)
            <x-button x-bind:class="{ 'bg-gray-700': collapsed }"
                class="w-full text-gray-300 group-hover:text-gray-200 hover:bg-gray-700 hover:text-gray-100 group flex items-center justify-between px-2 py-2 text-sm font-medium rounded-md mb-1"
                ...
                <div class="flex items-center">
                    @if ($toc_item->markdown_item->depth === 0 && $toc_item->markdown_item->is_dir)
                        @svg($toc_item->markdown_item?->icon, 'mr-2 flex-shrink-0 h-6 w-6')
