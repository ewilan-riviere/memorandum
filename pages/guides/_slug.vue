        ...
        <li class="mb-4">
          ...
          <ul>
            <li
              ...
              :key="guide.id"
              class="text-gray-700 transition-colors duration-300 rounded-md cursor-pointer dark:text-gray-300 hover:bg-green-200 hover:bg-opacity-50"
...
export default {
  ...
  async asyncData({ $content, params }) {
    const guides = await $content(
      `guides/${params.category}/${params.subCategory}`,
      { deep: true }
    )
      .sortBy('slug')
      .fetch()
