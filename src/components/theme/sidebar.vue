  ...
  <TransitionRoot as="template" :show="sidebarOpen">
    <Dialog as="div" class="fixed inset-0 flex z-40 lg:hidden" @close="sidebarOpen = false">
      ...
      <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
        ...
        <div class="relative flex-1 flex flex-col max-w-xs w-full bg-gray-900 focus:outline-none">
            ...
            <nav aria-label="Sidebar" class="mt-5">
              ...
              <div class="px-2 space-y-1">
                <a v-for="item in secondaryNavigation" :key="item.name" :href="item.href"
                  ...
                  <component :is="item.icon" class="text-gray-400 group-hover:text-gray-400 mr-4 flex-shrink-0 h-6 w-6"
                    aria-hidden="true" />
                  {{ item.name }}
                </a>
              </div>
          ...
          <div class="flex-shrink-0 flex border-t border-gray-700 p-4">
            <a href="#" class="flex-shrink-0 group block">
              <div class="flex items-center">
                ...
                <div class="ml-3">
                  <p class="text-base font-medium text-gray-300 group-hover:text-gray-100">
                    {{ user.name }}
                  </p>
