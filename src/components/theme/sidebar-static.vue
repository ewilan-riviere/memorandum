<script setup lang="ts">import { useContent } from '@/src/composables/useContent';
        ...
        <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
          ...
          <nav class="mt-5 flex-1" aria-label="Sidebar">
            <div class="px-2 space-y-1">
              ...
              <div v-for="category in navigation" :key="category.slug" class="p-1">
                <div class="category-title mb-2">
                  {{ category.label }}
                </div>
                <button v-for="domain in category.domains" :key="domain.slug" :class="['text-gray-400 hover:bg-gray-700 hover:text-gray-100',
                  'group flex items-center px-2 py-2 text-sm font-medium rounded-md w-full capitalize',
                ]" @click="setDomain(domain)">
                  <svg-icon :name="`sidebar/${domain.slug}`" :class="['text-gray-400 group-hover:text-gray-400',
                    'mr-3 flex-shrink-0 h-6 w-6',
                  ]" aria-hidden="true" />
                  {{ domain.label }}
                </button>
            ...
            <div class="flex-1 px-2 space-y-1">
              <a v-for="item in secondaryNavigation" :key="item.name" :href="item.href"
                ...
                <component :is="item.icon" class="text-gray-400 group-hover:text-gray-400 mr-3 flex-shrink-0 h-6 w-6"
                  aria-hidden="true" />
                {{ item.name }}
              </a>
            </div>
        ...
        <div class="flex-shrink-0 flex border-t border-gray-700 p-4">
          <a href="#" class="flex-shrink-0 w-full group block">
            <div class="flex items-center">
              ...
              <div class="ml-3">
                <p class="text-sm font-medium text-gray-300 group-hover:text-gray-100">
                  {{ user.name }}
                </p>
